...
int main() {
	int N, X;
	cin >> N >> X;
	vector<pair<ll,ll>> stage(N+1);
	rep1(i,N) cin >> stage[i].first >> stage[i].second;
	vector<ll> cost(N+1);
	ll costMin = numeric_limits<ll>::max();
	ll min = numeric_limits<ll>::max();
	for (int i = 1; i <= N; i++) {
		if (i > X) break; 
		cost[i] = cost[i-1] + stage[i].first + stage[i].second;
		chmin(costMin, stage[i].second);
		chmin(min, cost[i] + costMin*(X-i));
	}
	cout << min << endl;
	return 0;
}
